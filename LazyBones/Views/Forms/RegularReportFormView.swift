                ...
                VStack(alignment: .leading, spacing: 0) {
                    ...
                    VStack(spacing: 0) {
                        if selectedTab == .good {
                            TagWheelSectionView(
                                allTags: goodTags,
                                selectedIndex: $pickerIndexGood,
                                isTagAdded: { tag in
                                    goodItems.contains(where: { $0.text == tag.text })
                                },
                                onAddTag: { tag in
                                    addGoodTag(tag)
                                },
                                viewId: "regular_good_" + goodTags.map { $0.text }.joined(separator: "|")
                            )
                        } else {
                            TagWheelSectionView(
                                allTags: badTags,
                                selectedIndex: $pickerIndexBad,
                                isTagAdded: { tag in
                                    badItems.contains(where: { $0.text == tag.text })
                                },
                                onAddTag: { tag in
                                    addBadTag(tag)
                                },
                                viewId: "regular_bad_" + badTags.map { $0.text }.joined(separator: "|")
                            )
                        }
                    }
                    .padding(.vertical, 6)
